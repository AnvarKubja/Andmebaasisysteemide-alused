...
-- kui sisend on sama. Selle alla kuuluvad veel sum, abg, min, max, count
select square(3)
...
-
create function dbo.fn_GetEmployeeNameId(@id int)
returns nvarchar(20)
with schemabinding
as begin
...
--ei saa tabelit kustutada, kui sellel on schemabinding funktsioon
...
--Local on nähtav ainult ühendusele, kes lõi selle, kustutakse peale ühenduse lõpetamist
--Global on nähtav kõigile ja kustutatakse kui kõik ühendused on lõpetatud
...
-- klastris olev indeks on analoogne telefoni numbrile
...
--sisestame andmed tabelisse (ei saa)
...
drop index EmployeeFirstName.PK__Employee__3213E83F247C9392
-- kui käivitad ülavalpool oleva koodi, siis tuleb veateade
...
alter view vEmployeesByDepartment
--kustutamine
drop view vEmployeesByDepartment
...
as begin
	...
	--käib käbi kõik andmed temp tabelis
...
as
select Department.Id, DepartmentName, count(Employee.Id) as EmployeeCount
from Employee
join Department
on Employee.DepartmentId = Department.Id
...
--proovime info saada temp tabelist ja kus >= kahe töötajaga osakond
...
---Parem loetavus: CTE-d jagavad keerulsed päringud väiksemateks
...
--nüüd on veerg Id olemas, aga see ei mõjuta pivotit, kuna me ei 
...
-- lükatakse tagasi. Nt, mõlemad käsud peaksid õnnestuma. Andmebaas
--teeb selisel juhul: võtab esimese update tagasi ja veeretab selle algasendisse
...
--põhinevad sidumata tabelitel. Nt, muudatused kirjas, mis hiljem tagasi muudetakse.
...
--käsud ja andmed taastatakse algsesse olekusse. Taastamine toimub peale süsteemi
--taaskäivitamist.
...
--enamus juhtudel saab asendada subqueryt JOIN-iga
...
--teeme subquery, kus kasutame select-i. Kirjutame päringu, kus
--saame teada NAME ja TotalQuantity veeru andmeid.
Select Name,
(select sum(QuantitySold) from ProductSales where ProductId = Product.Id) as TotalQuantity
from Product
order by Name

-- sama tulemus JOIN-iga
...
-- transactioni näide, et mõlemad uuendatavad read peavad õnnestuma, et muudatused jääksid kehtima
begin try
	begin transaction
...
select * from GetProductsByPrice_ITVF(213, 345)
...
select * from GetCustomerOrderSummary_MSTVF(1)
...
as begin
	...
	select
		...
		ListPrice,
		case
			when ListPrice < 500 then 'Odav'
			when ListPrice between 500 and 1500 then 'Keskmine'
			else 'Kallis'
...
-- kus kohe peale esimest käsklust käivitan teise
-- 2 transaction
...
ja teine transcation uuendab neid andmeid esimese ning
teise käsu vahel esimese transcationi jooksutamise ajal
*/
...
-- samm nr 3
update TableB set Name = 'Mary Transaction 3' where Id = 1
commit tran
...
Tuvastamisel lõpetab DB-mootor deadlocki ja valib ühe lõime ohvriks.
Seejärel keeratakse deadlockiohvri tehing tagasi ja
...
DEADLOCK_PRIORITY
1. vaikimisi on see Normal-i peal
...
ressursi kulutamine, siis ohver valitakse juhuslikkuse alusel
*/
...
--nüüd käivitan esimeses serveris spTransaction 1 ja 
